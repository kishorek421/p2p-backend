...
import { GET_ASSETS_IN_USE, GET_USER_DETAILS } from "@/constants/api_endpoints";
...
const UserDetails = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserDevices = () => {
      api
        .get(GET_ASSETS_IN_USE + `?customerId=${userId}`)
        .then((response) => {
          setAssignedDevicesList(
            (response.data?.data ?? []).map((data: any) => {
              const assetData = {
                ...(data.assetMasterDetails ?? {}),
                userAssignedToDetails: {
